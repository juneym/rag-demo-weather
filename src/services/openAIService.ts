...
export class OpenAIService {
    ...
    public async makeAFriendlyWeatherMessage(data: any): Promise<string> {
        try {
            ...
            const userPrompt = "Generate a friendly weather message using farenheit based on the following data: "
                             + "\n\nWeather Data: " + JSON.stringify(data, null, 2)  
                             + "\n\nReturn only the message, no other text.";
            //console.log(`[DEBUG] User Prompt: ${userPrompt}`); // Debugging line to check the user prompt
